...
namespace zombie
{
    ...
    // hitbox相关
    uint32_t* get_hit_x(Zombie* zombie);
    uint32_t* get_hit_y(Zombie* zombie);
    uint32_t* get_hit_w(Zombie* zombie);
    uint32_t* get_hit_h(Zombie* zombie);

    // 实现的自带函数
	void ZombieInitialize(Zombie* zombie, uint32_t theRow, ZombieType theZombieType, BOOL theVariant, Zombie* theParentZombie, uint32_t theFromWave);
    void RiseFromGrave(Zombie* zombie, uint32_t theCol, uint32_t theRow);
}
